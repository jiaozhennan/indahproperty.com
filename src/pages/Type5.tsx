...
const type5Property = {
  ...
  area: "450-550㎡",
  bedrooms: 5,
  bathrooms: 5,image: "https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_16_9&prompt=Luxury%20hilltop%20villa%20with%20infinity%20pool%2C%20panoramic%20ocean%20view%2C%20tropical%20garden%2C%20modern%20architecture%2C%20Bali&sign=1352f725c3202ac1e5a6cbe66e5d81b0",
  featured: true,
  description: {
      ...
      <main className="flex-grow">
        ...
        <section className="bg-blue-900 text-white py-16">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              <h1 className="text-3xl md:text-5xl font-bold mb-4">{getPropertyValue(type5Property, 'title')}</h1>
              <p className="text-blue-100 text-lg max-w-2xl">
                {language === 'zh' 
                  ? '位于金巴兰山坡上的豪华别墅，俯瞰印度洋和巴厘岛壮丽日落，享受宁静与奢华的完美结合。' 
                  : language === 'id' 
                    ? 'Villa mewah di lereng bukit Jimbaran, dengan pemandangan Samudra Hindia dan matahari terbenam yang indah di Bali, nikmati perpaduan sempurna antara ketenangan dan kemewahan.' 
                    : 'Luxury villa on Jimbaran hillside, overlooking the Indian Ocean and Bali\'s magnificent sunsets, enjoying the perfect combination of tranquility and luxury.'
                }
              </p>
            </motion.div>
          </div>
        ...
        <section className="py-16 bg-white">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              {/* 项目主图片 */}
              <div className="relative rounded-2xl overflow-hidden shadow-xl mb-8 cursor-pointer" onClick={() => openImageModal(type5Property.image)}>
                <img 
                  src={type5Property.image} 
                  alt={getPropertyValue(type5Property, 'title')} 
                  className="w-full h-[500px] object-cover"
                />
                {type5Property.featured && (
                ...
                {activeTab === 'overview' && (
                  ...
                  >
                    <p className="text-gray-700 mb-6 leading-relaxed">
                      {getPropertyValue(type5Property, 'description')}
                    </p>
                    ...
                    <div className="bg-blue-50 p-6 rounded-xl mb-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[
                          { icon: 'fa-check-circle', text: language === 'zh' ? '山坡位置，俯瞰印度洋和壮丽日落' : language === 'id' ? 'Lokasi di bukit, menawarkan pemandangan Samudra Hindia dan matahari terbenam yang indah' : 'Hilltop location, overlooking the Indian Ocean and magnificent sunsets' },
                          { icon: 'fa-check-circle', text: language === 'zh' ? '现代简约设计与巴厘岛文化元素的完美融合' : language === 'id' ? 'Perpaduan sempurna antara desain modern minimalis dan elemen budaya Bali' : 'Perfect blend of modern minimalist design and Balinese cultural elements' },
                          { icon: 'fa-check-circle', text: language === 'zh' ? '无边泳池，享受极致度假体验' : language === 'id' ? 'Kolam renang infinity, nikmati pengalaman liburan yang luar biasa' : 'Infinity pool, enjoy an extraordinary vacation experience' },
                          { icon: 'fa-check-circle', text: language === 'zh' ? '私人停车位，便捷出行' : language === 'id' ? 'Parkir pribadi, memudahkan mobilitas' : 'Private parking, convenient mobility' },
                          { icon: 'fa-check-circle', text: language === 'zh' ? '热带花园环绕，私密性极佳' : language === 'id' ? 'Dikelilingi oleh taman tropis, sangat privat' : 'Surrounded by tropical garden, excellent privacy' },
                ...
                {activeTab === 'features' && (
                  ...
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                      {type5Property.features.map((feature, index) => (
                ...
                {activeTab === 'gallery' && (
                  ...
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {/* 主图片 */}
                      <div className="relative rounded-xl overflow-hidden shadow-lg cursor-pointer aspect-video" onClick={() => openImageModal(type5Property.image)}>
                        <img 
                          src={type5Property.image} 
                          alt={getPropertyValue(type5Property, 'title')} 
                      ...
                      {/* 其他图片 */}
                      {type5Property.gallery.map((image, index) => (
                        <div key={index} className="relative rounded-xl overflow-hidden shadow-lg cursor-pointer aspect-video" onClick={() => openImageModal(image)}>
                          <img 
                            src={image} 
                            alt={`${getPropertyValue(type5Property, 'title')} - Image ${index + 1}`} 
                            className="w-full h-full object-cover"
                          />
                          <div className="absolute inset-0 bg-black bg-opacity-0 hover:bg-opacity-10 transition-all flex items-center justify-center">
                            <span className="bg-white bg-opacity-0 hover:bg-opacity-90 text-white hover:text-blue-900 rounded-full p-3 transition-all opacity-0 hover:opacity-100">
                ...
                {activeTab === 'location' && (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[
                          { icon: 'fa-umbrella-beach', title: language === 'zh' ? '海滩' : language === 'id' ? 'Pantai' : 'Beach', distance: language === 'zh' ? '车程10分钟' : language === 'id' ? '10 menit berkendara' : '10 min drive' },
                          { icon: 'fa-utensils', title: language === 'zh' ? '餐厅' : language === 'id' ? 'Restoran' : 'Restaurants', distance: language === 'zh' ? '车程5分钟' : language === 'id' ? '5 menit berkendara' : '5 min drive' },
                          ...
                          { icon: 'fa-plane', title: language === 'zh' ? '机场' : language === 'id' ? 'Bandara' : 'Airport', distance: language === 'zh' ? '车程25分钟' : language === 'id' ? '25 menit berkendara' : '25 min drive' },
                          { icon: 'fa-school', title: language === 'zh' ? '国际学校' : language === 'id' ? 'Sekolah Internasional' : 'International School', distance: language === 'zh' ? '车程15分钟' : language === 'id' ? '15 menit berkendara' : '15 min drive' }
                        ].map((item, index) => (
